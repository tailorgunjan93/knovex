...
| 9 | "Failed to uninstall old application files.: 2" on install/update | **Real cause (the v0.10.0 `oneClick:false` theory was wrong):** the separate `knovex-backend.exe` (PyInstaller) keeps running during an update and holds open handles in the install dir. NSIS closes the Electron app but not this child process, so the old uninstaller hits "File is busy" → exit code 2 → `$(uninstallFailed): 2`. | v0.11.1 | ✅ `desktop/build/installer.nsh` (wired via `nsis.include`) force-kills `knovex-backend.exe` in `customInit` (before uninstallOldVersion) + `customUnInstall`. **Empirically verified by building the real installer:** install-over-existing with the backend running = exit 0 across repeated cycles; without the script = exit 2. Guard: `tests/test_installer_nsis.py` |
