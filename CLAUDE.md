...
| 20 | **Chat over-grounded on every message**: with a KB selected, saying "hi" produced a confabulated answer built from an irrelevant chunk (the model even invented a question — "your question about a 1-year-old child's breastfeeding accommodations"). User noticed it happened with **shiva-chat** (small local) but NOT **gpt-oss-120b** (large) — same pipeline, different instruction-following. | `stream_message` ran a **fixed flag-driven pipeline**: KB selected ⇒ *always* retrieve ⇒ `_build_messages` injected "Always prioritise information from the provided context." A weak model blindly obeys the forced grounding on an irrelevant chunk; a strong model ignores it. There was **no model-driven decision and no relevance gate** — retrieval was unconditional and grounding was mandatory. | unreleased (`feat/react-agent-chat`) | ✅ New capability-gated **ReAct agent** (`backend/core/agent/`): capable models (cloud, or large local like gpt-oss-120b — gated by **model size**, not just provider, so a 120B on Ollama still qualifies) run a bounded Thought→Action→Observation loop that *decides* whether to `search_kb`/`web_search`/`fetch_url`; small/local models (shiva-chat) take a **deterministic router** (greeting → no retrieval) + **relevance gate** (drop a chunk sharing no terms with the query). Explicit commands (`/news`, `/summarize`) and `agent_enabled=False` keep the legacy pipeline. Tool calls surface as `status` SSE → live "Searching…" trace (free engagement). Tests: `test_agent_capability.py` (21), `test_agent_tools.py` (10), `test_agent_router.py` (25), `test_agent_react.py` (6), `test_chat_agent_integration.py` (6, real HTTP — reproduces the shiva-chat "hi" bug fix + relevance gate over the wire). **Lesson:** retrieval must be conditional and grounding must be earned — never force a weak model to ground in unranked context; and capability tracks model *strength* (size), which a large local model has despite a "local" provider. |
