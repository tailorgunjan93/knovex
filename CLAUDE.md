...
| 10 | "LLM returned invalid JSON: Expecting ',' delimiter" on guided/animated (any JSON format) | LLM emits an **unescaped `"` inside a string value** (quoting a term mid-sentence, e.g. the "boundary" layer) → string terminates early, `json.loads` fails. Prior `_repair_truncated_json` only fixed truncation at the END, not stray mid-string quotes. `_escape_inner_quotes` uses comma/lookahead disambiguation (a real value-separating comma is followed by a key/value, not prose) so quoted-term-before-comma cases (common in Hindi prose) are recovered too; full raw payload now logged on unrepairable failures | unreleased (`design/visual-redesign-lab`) | ✅ `TestUnescapedInnerQuoteRepair` (12 tests) — `_escape_inner_quotes` + chained `_parse_llm_json` |
